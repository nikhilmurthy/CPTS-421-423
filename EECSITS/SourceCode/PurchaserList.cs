...
namespace NewITS
{
    public partial class frmListPurchaser : Form
    {
        DBA db = DBA.getInstance;
        List<string>[] la_purchaser;

        public frmListPurchaser()
        {
            InitializeComponent();
            load_datagrid();
        }
        ...
        public void load_datagrid()
        {
            ...
            rc = la_purchaser[0].Count;
            dataGridView1.RowCount = rc;
            int i = 0;
            while (i < rc)
            {
                dataGridView1.Rows[i].Cells[0].Value = la_purchaser[5][i];
                dataGridView1.Rows[i].Cells[1].Value = la_purchaser[1][i];
                dataGridView1.Rows[i].Cells[2].Value = la_purchaser[2][i];
                dataGridView1.Rows[i].Cells[3].Value = la_purchaser[3][i];
                dataGridView1.Rows[i].Cells[4].Value = la_purchaser[0][i];
                i++;
            }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            if (btn.Name == "btnExportCSV")
                rmsg = Global.exportToCsvFile(dataGridView1, "PurchaserList", "");
            else
                rmsg = Global.exportToPdfFile(dataGridView1, "PurchaserList", "");
